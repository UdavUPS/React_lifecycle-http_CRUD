...
function App() {
  ...
  async function xmlDel(id) {
    let kod;
    fetch(`http://localhost:7070/notes/${id}`,{
        method: 'DELETE'
    }).then(response => {if (response.status == 204) {refPage()}});
    
};

  // Функция удаления элемента
  async function closeMessage(e) {
 ...
 async function xmlPost(text) {
    ...
    }).then(res => {if (res.status == 204) {refPage()}});   
  ...
  return (
    <div className="app-body">
      <div className="app-body__header">
        <h1 className="app-body__header__title">CRUD <p className="refSimbol" onClick={refPage}>{refSimbol}</p></h1>
      </div>
      <div className="app-body__message-box">
        {notes.map((item) => (
          <MessageComponent key={item.id} mesID={item.id} messageText = {item.content} onClick={(e) => {closeMessage(e)}}/>
        ))}
      </div>
